...
// let paddingTop = safeAreaInsets ? safeAreaInsets.top : 20;
...
const orderTypes = [
  ...
  { type: '3', text: '已出票', icon: 'icon-gift' },
  // { type: '2', text: '待出票', icon: 'icon-gift' },
  { type: '4', text: '已取消', icon: 'icon-check' },
...
// 定义 switchToOrders 方法
const switchToOrders = (type: string) => {
  // 将参数存储到 localStorage 中
  // localStorage.setItem('orderType', type);
  uni.setStorageSync('orderType', type);

  // 切换到 tabBar 页面
  uni.switchTab({
    url: '/pages/ticketFolder/list'
  });
};
</script>
...
<template>
  <scroll-view enable-back-to-top @scrolltolower="onScrolltolower" class="viewport" scroll-y>
    ...
    <view class="profile" :style="{ paddingTop: paddingTop + 'px' }">
      <!-- 情况1：已登录 -->
    ...
    <view class="orders">
      <view class="title">
        ...
        <view class="navigator" @tap="() => switchToOrders('0')">
      ...
      <view class="section">
        ...
        <navigator v-for="item in orderTypes" :key="item.type" :class="item.icon" @tap="() => switchToOrders(item.type)"
          class="navigator">
          {{ item.text }}
        </navigator>
        <!-- 客服 -->
